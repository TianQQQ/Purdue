...
module sr_9bit ( clk, n_rst, shift_strobe, serial_in, packet_data, stop_bit );
  ...
  flex_stp_sr_NUM_BITS9_SHIFT_MSB0 SHIFT_REGISTER_9bit ( .clk(clk), .n_rst(
...
module flex_counter_NUM_CNT_BITS4_1 ( clk, n_rst, clear, count_enable, 
  ...
  wire   next_roll_flag, n40, n41, n42, n43, n1, n2, n3, n4, n5, n6, n7, n8,
         n14, n15, n16, n17, n18, n19, n20, n21, n22, n23, n24, n25, n26, n27,
         n28, n29, n30, n31, n32, n33, n34, n35, n36, n37, n38, n39, n44, n45,
         n46, n47, n48, n49, n50, n51, n52;

  DFFSR \curr_count_reg[0]  ( .D(n43), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        count_out[0]) );
  DFFSR \curr_count_reg[1]  ( .D(n42), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        count_out[1]) );
  DFFSR \curr_count_reg[2]  ( .D(n41), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        count_out[2]) );
  DFFSR \curr_count_reg[3]  ( .D(n40), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        count_out[3]) );
  DFFSR curr_roll_flag_reg ( .D(next_roll_flag), .CLK(clk), .R(n_rst), .S(1'b1), .Q(rollover_flag) );
  NOR2X1 U7 ( .A(n1), .B(n2), .Y(next_roll_flag) );
...
module flex_counter_NUM_CNT_BITS4_0 ( clk, n_rst, clear, count_enable, 
  ...
  input clk, n_rst, clear, count_enable;
  output rollover_flag;
  wire   next_roll_flag, n1, n2, n3, n4, n5, n6, n7, n8, n14, n15, n16, n17,
         n18, n19, n20, n21, n22, n23, n24, n25, n26, n27, n28, n29, n30, n31,
         n32, n33, n34, n35, n36, n37, n38, n39, n44, n45, n46, n47, n48, n49,
         n50, n51, n52, n53, n54, n55, n56;

  DFFSR \curr_count_reg[0]  ( .D(n53), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        count_out[0]) );
  DFFSR \curr_count_reg[1]  ( .D(n54), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        count_out[1]) );
  DFFSR \curr_count_reg[2]  ( .D(n55), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        count_out[2]) );
  DFFSR \curr_count_reg[3]  ( .D(n56), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        count_out[3]) );
  DFFSR curr_roll_flag_reg ( .D(next_roll_flag), .CLK(clk), .R(n_rst), .S(1'b1), .Q(rollover_flag) );
  NOR2X1 U7 ( .A(n1), .B(n2), .Y(next_roll_flag) );
  NAND3X1 U8 ( .A(count_enable), .B(n3), .C(n4), .Y(n2) );
...
module timer ( clk, n_rst, enable_timer, shift_strobe, packet_done );
  ...
  wire   enable_delay, ff;

  DFFSR ff_reg ( .D(enable_timer), .CLK(clk), .R(n_rst), .S(1'b1), .Q(ff) );
  DFFSR enable_delay_reg ( .D(ff), .CLK(clk), .R(n_rst), .S(1'b1), .Q(
        enable_delay) );
  flex_counter_NUM_CNT_BITS4_1 count_to_9 ( .clk(clk), .n_rst(n_rst), .clear(
        packet_done), .count_enable(shift_strobe), .rollover_val({1'b1, 1'b0, 
        1'b0, 1'b1}), .rollover_flag(packet_done) );
  flex_counter_NUM_CNT_BITS4_0 count_to_10 ( .clk(clk), .n_rst(n_rst), .clear(
        packet_done), .count_enable(enable_delay), .rollover_val({1'b1, 1'b0, 
...
module rcu ( clk, n_rst, start_bit_detected, packet_done, framing_error, 
  ...
  DFFSR \currentState_reg[0]  ( .D(nextState[0]), .CLK(clk), .R(n_rst), .S(
        1'b1), .Q(currentState[0]) );
  DFFSR \currentState_reg[2]  ( .D(nextState[2]), .CLK(clk), .R(n_rst), .S(
        1'b1), .Q(currentState[2]) );
  DFFSR \currentState_reg[1]  ( .D(nextState[1]), .CLK(clk), .R(n_rst), .S(
        1'b1), .Q(currentState[1]) );
  OAI21X1 U6 ( .A(n4), .B(n5), .C(n6), .Y(nextState[2]) );
  NAND2X1 U7 ( .A(currentState[2]), .B(n7), .Y(n5) );
  NAND2X1 U8 ( .A(n8), .B(n9), .Y(n4) );
  OR2X1 U9 ( .A(sbc_clear), .B(enable_timer), .Y(nextState[1]) );
  NOR2X1 U10 ( .A(n7), .B(currentState[2]), .Y(sbc_clear) );
  OAI21X1 U11 ( .A(n10), .B(n11), .C(n12), .Y(nextState[0]) );
  NAND2X1 U12 ( .A(packet_done), .B(enable_timer), .Y(n12) );
  NAND2X1 U13 ( .A(n13), .B(n8), .Y(n11) );
  MUX2X1 U14 ( .B(start_bit_detected), .A(n9), .S(currentState[2]), .Y(n10) );
  INVX1 U15 ( .A(framing_error), .Y(n9) );
  INVX1 U16 ( .A(n6), .Y(sbc_enable) );
  NAND3X1 U17 ( .A(currentState[0]), .B(n14), .C(currentState[1]), .Y(n6) );
  INVX1 U18 ( .A(n15), .Y(enable_timer) );
  NAND3X1 U19 ( .A(n13), .B(n14), .C(currentState[1]), .Y(n15) );
  INVX1 U20 ( .A(currentState[0]), .Y(n13) );
  NOR2X1 U21 ( .A(n7), .B(n14), .Y(load_buffer) );
  INVX1 U22 ( .A(currentState[2]), .Y(n14) );
  NAND2X1 U23 ( .A(currentState[0]), .B(n8), .Y(n7) );
  INVX1 U24 ( .A(currentState[1]), .Y(n8) );
endmodule
...
module rcv_block ( clk, n_rst, serial_in, data_read, rx_data, data_ready, 
  ...
  wire   start_bit_detected, shift_strobe, stop_bit, enable_timer, packet_done,
         load_buffer, sbc_clear, sbc_enable;
  wire   [7:0] packet_data;

  start_bit_det START_BIT_DECTOR ( .clk(clk), .n_rst(n_rst), .serial_in(
  ...
  stop_bit_chk STOP_BIT_CHECKING ( .clk(clk), .n_rst(n_rst), .sbc_clear(
