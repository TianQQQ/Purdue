...
typedef struct mbox {
  Queue msgQueue;            //msg buffer queue
  Queue pidQueue;            //pid holding the mbox
  int inuse;
  lock_t lockHandle;          //lock to change the inuse stage
  sem_t semMsgNFull;          //sem will wait when full
  sem_t semMsgNEmpty;         //sem will wait when empty
} mbox;
