...
typedef struct PCB {
  ...
  int           lastStartJiffie;  // Store the start time of current quanta
  int           totalJiffies;     // Store the total time spent on this process
  int           lastSleepJiffie;  // Store the start of last sleep jiffie
  int           wakeJiffie;

  int           decayFlag;      //mark the pcb in the current globle decay.
...
#define RUN_QUEUE_SIZE MAX_PRIORITY/PRIORITY_PER_QUEUE    //equals to MAX_PRIORITY
#define JIFFIES_PER_QUANTA 11
