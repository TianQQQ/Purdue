...
// to happen).
static Queue	runQueue[RUN_QUEUE_SIZE];
...
inline int WhichQueue(PCB *pcb){
  return pcb->priority/4;
}

int ProcessInsertRunning(PCB *pcb){}

void ProcessDecayEstcpu(PCB *pcb){
  // Increment estCpu when the process used an entire window of cpu time
  if(ClkGetCurJiffies() - pcb->lastStartJiffie == JIFFIES_PER_QUANTA)
    pcb->estCpu++;
  ProcessRecalcPriority(pcb);
  return;
}

void ProcessDecayEstcpuSleep(PCB *pcb, int timeAsleepJiffies){
  int i;
  int numWindowsAsleep;
  //  printf("timeAsleep: %d\n", timeAsleepJiffies);
  if(timeAsleepJiffies >= 10*10){
    numWindowsAsleep = timeAsleepJiffies / (10 * 10);
    for(i = 0;i < numWindowsAsleep;++i){
      pcb->estCpu *= 2.0/3.0;
    }
  }
  ProcessRecalcPriority(pcb);
...
PCB * ProcessFindHighestPriorityPCB(){
  ...
  for(i = 0;i < RUN_QUEUE_SIZE;++i){
    if (!AQueueEmpty(&runQueue[i])) {
      ...
      if(pcb == IdlePCB && AQueueLength(&runQueue[i]) != 1){
...
void ProcessPrintRunQueues(){
  ...
  while(l!=NULL){
    pcb = (PCB*) AQueueObject(l);
    printf("wait name: %s, estCpu: ", pcb->name);
    printf("%f", pcb->estCpu);
    printf("\n");
    l = AQueueNext(l);
  }
...
void ProcessSchedule () {
  ...
  Link *l=NULL;
  int queueNum;
  int noRunningProcess;
  int noAutoWakingProcess;

  dbprintf ('p', "Now entering ProcessSchedule (cur=0x%x)\n", (int)currentPCB);

  //autowakeup
  noAutoWakingProcess = 1;
  l = AQueueFirst(&waitQueue);
  while (l != NULL) {
    pcb = AQueueObject(l);
    l = AQueueNext(l);
    if(pcb->flags & PROCESS_STATUS_AUTOWAKE){
      noAutoWakingProcess = 0;
      if(pcb->wakeJiffie <= ClkGetCurJiffies())
	ProcessWakeup(pcb);
    }
  }
  ...
  for(i = 0;i < RUN_QUEUE_SIZE;++i){
    if (!AQueueEmpty(&runQueue[i])) {
      noRunningProcess = 0;
      if (i == RUN_QUEUE_SIZE - 1 && AQueueLength(&runQueue[i])  == 1)
	noRunningProcess = 1;
      break;
    }
  }

  if (noRunningProcess && noAutoWakingProcess) {
    if (AQueueLength(&waitQueue)) {
      printf("FATAL ERROR: no runnable & autowaking processes, but there are sleeping processes waiting!\n");
      l = AQueueFirst(&waitQueue);
      while (l != NULL) {
	pcb = AQueueObject(l);
  ...
  // Update old pcb(currpcb) total time
  currentPCB->totalJiffies += ClkGetCurJiffies() - currentPCB->lastStartJiffie;

  // Print process info according to pinfo flag.
  if(currentPCB->pinfo == 1){
    printf(PROCESS_CPUSTATS_FORMAT, GetCurrentPid(), currentPCB->totalJiffies, currentPCB->priority);
  }
  if(currentPCB->flags & (PROCESS_STATUS_RUNNABLE | PROCESS_STATUS_YIELD)){
    // Remove currentpcb fron its run queue
    ...
    // Push the currentPCB to the end of its new runQueue
  ...
  // Update cpu window count
  windowCount++;
  // Decay all if meets 10 windows
  if(windowCount == 10){    
    windowCount = 0;
    ProcessDecayAllEstcpus();
  }
  // Now, run the one with the highest priority;
  pcb = ProcessFindHighestPriorityPCB();

  // If after the global decay, there are other processes come to the first group.
  // Put current pcb to the end.
  if (currentPCB == pcb){
    AQueueRemove(&currentPCB->l);
    queueNum = WhichQueue(currentPCB);
    // Push the currentPCB to the end of its new runQueue
  ...
  currentPCB = pcb;
  // Set Time info for next pcb 
  pcb->lastStartJiffie = ClkGetCurJiffies();
  //  printf("Curr j: %d ********\n", ClkGetCurJiffies());
...
void ProcessSuspend (PCB *suspend) {
  ...
  suspend->lastSleepJiffie = ClkGetCurJiffies();
...
void ProcessWakeup (PCB *wakeup) {
  ...
  ProcessDecayEstcpuSleep(wakeup, ClkGetCurJiffies() - wakeup->lastSleepJiffie);
...
//	ProcessIdle
//
//	This routine is called to create an idle process
//
//----------------------------------------------------------------------
...
void main (int argc, char *argv[])
{
  ...
  IdlePCB = &pcbs[ProcessFork(&ProcessIdle, 0, 0, 0, (char *)"IdleProcess", 0)];
...
void ProcessUserSleep(int seconds) {
  ...
  currentPCB->wakeJiffie = ClkGetCurJiffies() + seconds*(1000000/ClkGetResolution());
}
