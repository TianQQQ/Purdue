...
lock_t lock_create();			//trap 0x453 Calls LockCreate()
int lock_acquire(lock_t lock);		//trap 0x454 Calls LockHandleAcquire()
int lock_release(lock_t lock);		//trap 0x455 Calls LockHandleRelease()

cond_t cond_create(lock_t lock);	//trap 0x456 Calls CondCreate()
int cond_wait(cond_t cond);		//trap 0x457 Calls CondHandleWait()
int cond_signal(cond_t cond);		//trap 0x458 Calls CondHandleSignal()
int cond_broadcast(cond_t cond);	//trap 0x459 Calls CondHandleBroadcast()
