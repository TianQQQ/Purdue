...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
	...
	// Init user stack
	pcb->pagetable[MEM_PAGETABLE_SIZE-1] = MemorySetupPte(MemoryAllocPage());

	// Init 4 page for code and global
	...
	// Init 1 page for heap
	pcb->pagetable[4] = MemorySetupPte(MemoryAllocPage());
	pcb->heapAddrBase = 4 << MEM_L1FIELD_FIRST_BITNUM;
	...
	stackframe[PROCESS_STACK_PTSIZE] = MEM_PAGETABLE_SIZE;
...
//	Create a new process and make it runnable.  This involves the
//	following steps:
//	* Allocate resources for the process (PCB, system stack etc.)
//	* Copy PCB data into new PCB
//      * Set all pages read only bit
//      * Modify sysStackPtr, currentSavedFrame, 
//          PROCESS_STACK_PTBASE for new PCB
//	* Place the PCB on the runnable queue
//
//
//----------------------------------------------------------------------
...
int ProcessRealFork(){
	...
	// Set pte readonly
	for (i = 0;i < MEM_PAGETABLE_SIZE;++i){
		page = MemoryPteSetROIfValid(currentPCB->pagetable[i]);
		if(page & MEM_PTE_VALID){
			currentPCB->pagetable[i] = page;
			MemoryIncreasePageUsageCounter(currentPCB->pagetable[i]);
			//      printf("old virPage: %x, phypage: %x\n", i, page);
		}
	}
	// Copy pcb info
  ...
  dstrcpy(pcb->name, currentPCB->name);

  // Set pte readonly
  for (i = 0;i < MEM_PAGETABLE_SIZE;++i){
    currentPCB->pagetable[i] = MemoryPteSetROIfValid(currentPCB->pagetable[i]);
    pcb->pagetable[i] = currentPCB->pagetable[i];
    MemoryIncreasePageUsageCounter(pcb->pagetable[i]);
  }
  pcb->npages = currentPCB->npages;
	 */
	//----------------------------------------------------------------------
	// This section initializes the memory for this process
	//----------------------------------------------------------------------
	// Allocate 1 page for system stack, copy all other pages to new pte
	// and set both pte pages to read only

	// Init sys stack
	pcb->sysStackArea = MemoryAllocPage();

	// Copy system stack here %%%
	bcopy((char *)currentPCB->sysStackArea, (char *)pcb->sysStackArea, MEM_PAGESIZE);
	stackframe = (uint32 *)(pcb->sysStackArea | ((1 << MEM_L1FIELD_FIRST_BITNUM) - 4));
	stackframe -= PROCESS_STACK_FRAME_SIZE;

	// keep offset and replace to new sysStackPageNum
	pcb->currentSavedFrame = (uint32 *)(pcb->sysStackArea | ((uint32)currentPCB->currentSavedFrame & MEM_ADDRESS_OFFSET_MASK));
	pcb->sysStackPtr = (uint32 *)(pcb->sysStackArea | ((uint32)currentPCB->sysStackPtr & MEM_ADDRESS_OFFSET_MASK));

	/*  printf("csf: %x, ssp %x\n", currentPCB->currentSavedFrame, currentPCB->sysStackPtr);
      printf("csf: %x, ssp %x\n", pcb->currentSavedFrame, pcb->sysStackPtr);*/

	stackframe[PROCESS_STACK_PTBASE] = (uint32) (pcb->pagetable);


	PrintPageTableEntries(pcb, "New Process after Fork");
	PrintPageTableEntries(currentPCB, "old Process after Fork");
	// Place the PCB onto the run queue.
	intrs = DisableIntrs ();
	if ((pcb->l = AQueueAllocLink(pcb)) == NULL) {
...
void PrintPageTableEntries(PCB* pcb, char *pcbName){
	int i;
	printf("%s\n", pcbName);
	for (i = 0;i < MEM_PAGETABLE_SIZE;++i){
		if (pcb->pagetable[i] & 1)
			printf("virPage: %x, phypage: %x\n", i, pcb->pagetable[i]);
	}
}
