...
void* malloc(PCB *pcb, int memsize){
	int order;
	int minOrder = MEM_HEAP_MAX_ORDER+1, minOrderLocation = 0;
	int i;
	int upperBound;
	...
	//find memsize order
	if(memsize < MEM_HEAP_BLOCK_SIZE)
		order = 0;
	else
		for(order = 0;order <= MEM_HEAP_MAX_ORDER;++order){
			if (((memsize-1) >> (MEM_HEAP_FIRST_BITNUM + order) == 0) && (((memsize-1) >> (MEM_HEAP_FIRST_BITNUM + order - 1)) != 0)){
				break;
			}
		}
	...
	// No available space
	if(minOrder == MEM_HEAP_MAX_ORDER+1)
		return NULL;
...
int mfree(PCB *pcb, void* ptr){
	...
	while(1){
		...
		for (i = parentStart;i < parentEnd;++i){
			pcb->heapTree[i] = MEM_HEAP_ORDER_MASK & order;
		}
		...
		printf("\n");*/
		if((pcb->heapTree[parentStart] & MEM_HEAP_USAGE_MASK) != 0
				|| (pcb->heapTree[parentEnd-1] & MEM_HEAP_USAGE_MASK) != 0
				|| (pcb->heapTree[parentStart] & MEM_HEAP_ORDER_MASK) != order
				|| (pcb->heapTree[parentEnd-1] & MEM_HEAP_ORDER_MASK) != order){
			break;
		}
		order++;
