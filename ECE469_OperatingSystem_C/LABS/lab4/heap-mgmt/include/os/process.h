...
typedef struct PCB {
  ...
  uint32    heapAddrBase;
  char      heapTree[128];   //For each block, MSB indicates whether the block is using, rest 7 bits
  	 	 	 	 	 	 	 // indicates the order of a block
  	  	  	  	  	  	  	 // for example, 8b10000011 indicates a order 3 block is using
  	  	  	  	             // 8b00000110 indicates a order 6 block is not using
  unsigned int	flags;
  char		name[80];	// Process name
  //  uint32	pagetable[/* Put the size of the L1 page table here */];
