...
*/ 	   			 		  			 		  		
  MODRR = 8;   // PT3 used as PWM Ch3 output
  PWME = 8;   // enable PWM ch3
  PWMPOL = 0; // set active low polarity
  PWMCTL = 0; // 8bit mode
  PWMCAE = 0; // left aligned output mode
  PWMPER3 = 0xFF; // set max = 8bit period
  PWMDTY3 = 0;  // initially clear duty register
  PWMCLK = 0x08;  // set clock sb for channel 03
  PWMPRCLK = 0x50; // set clock b = 7.5MHz (Prescalar 32) 
  PWMSCLB = 0x0F;   
...
/* Initialize digital I/O port pins */
  DDRT = 0x77;    // program port T for output mode
...
void rdisp()
{
   ...
   chgline(LINE1);
   pmsglcd("RPM = ");
   print_c((pulscnt/100)%10 + '0');
   print_c((pulscnt/10)%10 + '0');
   print_c((pulscnt%10) + '0');
   chgline(LINE2);   
   ...
   if(b == 10) {
      ...
      if (pulscnt > 240) {
        ...
        duty = pulscnt * 10 / (MAXRPM/10);
        print_c((duty/10)%10 + '0');
        print_c((duty%10) + '0');
        print_c('%');
      }
   }
