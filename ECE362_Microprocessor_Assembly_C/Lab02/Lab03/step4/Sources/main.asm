...
minmax  
      ...
      inca			; (A)=(A)+$01
      puly			; (SP)=>(Y)
      movw #$ffff,7,SP		; (M:M+1(1))=>M:M+1(2) memory to memory word-Move(16-bit)
for_loopD
      dbeq a,for_endD		; (a)-1=>(a) branch to for_endD if (center=0)
      ldx  0,y			; ?(M:M+1)=>X M:M+1 is 0 offset of y
      cpx  5,SP			; (X)-(M:M+1) 5 offset of sp
      bls  checkMin		; branch to checkMin if lower or same (C+Z=1)(unsigned)
      stx  5,SP			; (X)=>(M:M+1) 5 offset of sp
checkMin
      cpx  7,SP			; (X)-(M:M+1) 7 offset of sp
      bhi  checkedEverything	; branch if higher (C+Z=0)
      stx  7,SP			; (X)=>(M:M+1) 7 offset of sp
checkedEverything
      iny			; y++
      iny			; y++
      bra for_loopD		; branch to for_endD
for_endD
      pshy			; (YH:Yl)=>(SP,SP+1)
      psha
      pshx
