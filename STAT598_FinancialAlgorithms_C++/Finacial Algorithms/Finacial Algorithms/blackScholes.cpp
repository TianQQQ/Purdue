...
class BlackScholes{
    double S, sigma, K, r, d1, d2;
    int T;
    string types;
    
public:

    BlackScholes(double price, double volatility, double strike, double rate, int time_to_maturity, string type){
        S = price,
        sigma = volatility,
        K = strike,
        r = rate;
        ...
        d1 = (log(S / K) + r + pow(sigma, 2)*T / 2) / (sigma*sqrt(T));
        d2 = d1 - sigma*sqrt(T);
    }
