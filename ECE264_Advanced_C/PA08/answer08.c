...
List * List_createNode(const char * str){
	...
	list -> str = strdup(str); // string duplicate
	list -> next = NULL;
	return list;
}
...
List * List_merge(List * lhs, List * rhs, int (*compar)(const char *, const char*)){
	...
	ptr = NULL;
		...
		if(lhs == NULL){
	   	...
	   	ptr = tail;  // refresh tail 
	    }
      	while(lhs != NULL && rhs != NULL)  // 为何还要while一次 : 之前是初始化 
		{
	  	int cmp = compar(lhs->str,rhs->str);
	  	if(cmp <= 0){
	    	ptr->next = lhs;
...
List * List_sort(List * list, int (*compar)(const char *, const char*)){
	...
	if(len < 2){
    	...
    	lhs = List_sort(lhs, compar);  // split and compare
    	rhs = List_sort(rhs, compar);  // split and compare
    
    	list = List_merge(lhs,rhs, compar); // merge parts
    }
